...
|---|---|---|---|
| `networks` | `string[]` | No | Array of lowercase slugs. e.g. `["base"]`, `["ethereum", "arbitrum"]`. For unified search pass all in one array (`limit: 50`). For fair cross-network comparison, run one call per network in parallel (`limit: 20` each) — see output-formats.md. |
