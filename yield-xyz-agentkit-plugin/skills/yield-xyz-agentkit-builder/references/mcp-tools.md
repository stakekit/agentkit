...
This file enumerates **every tool exposed by the `yield-xyz-agentkit` / `yield-xyz` MCP server** and tells the builder skill exactly which ones to use, which ones to avoid, and why.
...
- **If the user wants to actually run / test action tools**, redirect them to the
  ...
  `yield-xyz-agentkit` for no-wallet exploration, `yield-xyz-agentkit-privy` /
  `yield-xyz-agentkit-moonpay` for execution).
...
User wants to actually run/test an
  action tool (enter/exit/manage)?  → Out of scope for the builder skill. Redirect to a
                                       dedicated skill (yield-xyz-agentkit / -privy / -moonpay)
