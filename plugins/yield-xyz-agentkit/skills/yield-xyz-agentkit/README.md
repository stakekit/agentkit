...
Look for `yield-xyz-agentkit` under available skills. Or ask directly:
...
```
Re-run setup for the yield-xyz-agentkit skill
```
...
```bash
# Remove skill
rm -rf ~/.claude/skills/yield-xyz-agentkit
...
- [Yield.xyz AgentKit × MoonPay Skill](../yield-xyz-agentkit-moonpay/README.md) — full end-to-end skill with wallet signing