...
The CLI will list all available skills, pick `yield-xyz-agentkit-moonpay`


Open your AI Agent and say:

```
Set up the yield-xyz-agentkit-moonpay skill
```
...
Both `yield-xyz-agentkit` and `moonpay` should appear under connected MCP servers.
...
```bash
claude mcp list
# Should show: yield-xyz-agentkit, moonpay (if using claude)
```
...
/context
```
Look for `yield-xyz-agentkit-moonpay` in the skills list.
...
└── references/
    ...
    └── key-rules.md            # MoonPay signing rules, transaction ordering
```
...
- [Yield AgentKit Skill](../yield-xyz-agentkit/) — Yield.xyz skill