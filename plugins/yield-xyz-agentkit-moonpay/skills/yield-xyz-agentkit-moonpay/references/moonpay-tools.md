...
- **Portfolio-aware discovery:** it enumerates the wallet's tokens, so feed the non-zero holdings into `yields_get_all` as `inputTokens` to fetch only enterable yields (see the base skill's "Canonical discovery")
