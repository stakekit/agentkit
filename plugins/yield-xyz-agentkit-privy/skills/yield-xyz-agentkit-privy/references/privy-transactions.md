...
```
TX stepIndex=0: use nonce as-is → Privy signs → broadcast → submit_hash → poll to terminal
TX stepIndex=1: increment nonce by 1 → Privy signs → broadcast → submit_hash → poll to terminal
TX stepIndex=2: increment nonce by 2 → Privy signs → broadcast → submit_hash → poll to terminal
```
